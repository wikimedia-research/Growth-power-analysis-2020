...
## This CI covers the values of retention rate found in the 2018 dataset used to estimate mu,
## so that seems fine.
...
par(mfrow = c(1, 2))
plot(NA, NA, xlim = c(0, 1), ylim = c(0.05, 0.25), xlab = "is_mobile", ylab = "prop_ret",
...
plot(NA, NA, xlim = c(-2, 0), ylim = c(-0.75, 0.25), xlab = "intercept", ylab = "slope",
...
## I don't see any correlation between intercept and slope in this case.
alpha_beta_rho <- 0.0

## We see that three of the four wikis have negative association between mobile
## and retention, whereas Korean has a positive association. That leads us to
## choose a sigma_mob that can model both a positive and negative impact.
## The chosen value has a slightly higher upside than the data, and a slightly
## lower downside than the data. We have to compromise, though, so we compromise.
sigma_mob <- 0.15
...
act_counts_first4 <- user_data[wiki_db %in% wikis,
                               list(n_activated = sum(n_activated)),
                               by = wiki_db]$n_activated

save(list = c("ret_coeff_first4", "act_counts_first4"),
     file = "simulation_parameters/first4_retention_params.txt",
     ascii = TRUE)
