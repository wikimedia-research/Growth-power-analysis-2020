## Improved power analysis: Accounting for mobile/desktop registrations.

## This analysis is based on the following assumptions:
## 1: We deploy an intervention to a set of wikis. This intervention is expected to have
##    a 10% increase in activation or retention (but not both), and this increase is uniform
##    across all wikis and whether the user is on the mobile or desktop site.
## 2: New registrations have a 50% chance of getting the treatment, and selection into the
##    treatment group happens at random.
## 3: Each wiki has an expected proportion of mobile registrations. The overall proportion
##    and its variation is estimated based on real data. New registrations then have some
##    chance of registering on the mobile site based on that proportion/variation.
## 4: Each wiki has an activation/retention rate depending on whether the user registered on
##    the desktop or mobile site. We estimate the activation/retention rate for the desktop site
##    from real data and use that as the default, then estimate a modifier for mobile.
## 5: We account for the variation activation/retention for the desktop and mobile site for
##    each wiki. In other words, it's a group effect in the multilevel regression.
...
## This CI covers (more or less) the values of activation rate found in the dataset
## used to estimate mu, so that seems fine.
...
plot(NA, NA, xlim = c(-2, 0), ylim = c(-0.5, 0.5), xlab = "intercept", ylab = "slope",
...
## It's difficult to determine if there's actually a correlation since we only have
## four wikis, so we set it at 0.
alpha_beta_rho <- 0.0

## We see quite a bit of variation in the effect of mobile registration, but it
## is always positive. This leads us to choose a sigma that doesn't render
## beta_mob negative.
...
reg_counts_first4 <- user_data[wiki_db %in% wikis,
                               list(n_registered = sum(n_registered)),
                               by = wiki_db]$n_registered/30

save(list = c("act_coeff_first4", "reg_counts_first4"),
     file = "simulation_parameters/first4_params.txt",
     ascii = TRUE)
