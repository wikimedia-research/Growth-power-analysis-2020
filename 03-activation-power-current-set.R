## In this scenario, we're looking at the eight wikis we've currently
## deployed our interventions to.
...
## This CI covers the values of activation rate found in the dataset
...
## In this case, it looks like we have a negative correlation
## as the intercepts closer to 0 are lower than those further away.
## Let's calculate a correlation:
...
## Still, -0.55. I think we'll be conservative and set it at -0.5.
alpha_beta_rho <- -0.5
...
reg_counts_current <- user_data[wiki_db %in% wikis,
                               list(n_registered = sum(n_registered)),
                               by = wiki_db]$n_registered/30

save(list = c("act_coeff_current", "reg_counts_current"),
     file = "simulation_parameters/current_params.txt",
     ascii = TRUE)
