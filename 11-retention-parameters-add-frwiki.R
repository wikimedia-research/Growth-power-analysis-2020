...
## In this case, it seems we have a fairly strong correlation for seven of
...
## We find a strong correlation of -0.56. I'll choose to be slightly
## conservative here, though, and set it to -0.5
alpha_beta_rho <- -0.5
...
act_counts_addfr <- user_data[wiki_db %in% wikis,
                               list(n_activated = sum(n_activated)),
                               by = wiki_db]$n_activated

save(list = c("ret_coeff_addfr", "act_counts_addfr"),
     file = "simulation_parameters/addfr_retention_params.txt",
     ascii = TRUE)